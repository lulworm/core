...
- [Development guidelines](#development-guidelines)
    - [General WISPR Core](#general-wispr-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
WISPR Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
WISPR Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/lulworm/core/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_wispr
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_wispr --log_level=test_suite
$ valgrind -v --leak-check=full src/wisprd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_wispr.coverage/index.html`.
```
...
be sent upstream but bugfixes may also be prudent to PR against WISPR Core so that they can be integrated
...
file descriptors it will cause problems with WISPR's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:WisprProject/core.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `wispr-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `wispr-cli` and the GUI debug console use this table to determine how to
